...
- **macOS 菜单栏图标**：`asset/image/menu_bar_icon.png`（纯黑 + 透明底的模板图），由 `script/build_menu_bar_icon.py` 从 `poc/mask-upright.svg` 生成，请勿手改
...
- **单个源文件尽量不超过 1000 行**，接近上限就按职责拆分。
- 新增功能优先落到新文件（沿用 `lib/page/<feature>/{bloc,view,widgets,models}` 的分层），通过 barrel 文件与最小挂载点接入，而不是往已经很大的文件末尾继续堆。
- 修改 800 行以上的文件时，顺手把本次涉及的那块抽成独立文件；不要为了压行数做与当前任务无关的大范围重排。
- 生成物（`*.g.dart`、`*.freezed.dart`、`frb_generated*`、`router.gr.dart` 等）不计入该约束。

### 6.6 提交粒度

- 每完成一个功能就及时提交，不要把多个功能的改动攒在同一次提交里。
- 提交时**只暂存该功能相关的文件**（逐个 `git add <path>`，不要用 `git add -A` / `git add .`）；工作区中无关的修改即使已有改动，也保留在工作区不提交。
- 提交前后各执行一次 `git status` 确认范围；无法判断某个文件是否属于本功能时，先询问而不是猜着提交。
...
| 修改 macOS 菜单栏图标 | `poc/mask-upright.svg`、`script/build_menu_bar_icon.py`、`lib/platform/desktop/system_tray.dart` |
