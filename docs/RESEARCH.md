...
Rossi 基于上游 `deretame/Breeze`，目标不是重新制作一个漫画客户端，而是在已有 Flutter + Rust + QuickJS + 超分基础上，逐步形成：
...
Flutter 本身不是 WebView UI；桌面端有 Flutter engine/Impeller，并提供 Texture/GPU 相关能力，因此具备承担高性能 Reader UI 的底层条件。但“Flutter 有 GPU”不等于“默认 Flutter Image 就达到 mImageViewer”。
...
Rossi 第一阶段继续使用 Breeze 插件运行时与现有插件生态 —— 保留它作为第一优先级的理由是
Breeze 已经具备在线图源生态与 QuickJS runtime。Mangayomi 与 Breeze 的插件协议不是直接兼容的；后续可以建立：
...
插件兼容层不是第一阶段任务，避免在 GPU Reader 验证之前扩大改造范围。
