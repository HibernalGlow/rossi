...
12. **顶栏按「平台有没有指针」分流：桌面悬停揭示，触摸屏常驻（同日第五轮）。**
    ...
    > **同日第七轮修正：顶栏本身改成「默认不画」，两种形态都归到开关底下。**
    > 新增 `WorkspaceInteractionSettings.showTopChrome`（默认 **false**），关掉时
    > 揭示浮层**整个不挂**（不是调透明 —— 那会留一层看不见却吃鼠标的东西）。
    > 第 10 / 12 条那段「第二层顶栏」的理由就是这一轮按下默认关的依据；
    > 而第 12 条立的那条不变量（触摸屏必须常驻，否则进得来出不去）被用户明确放宽成
    > 「全关」—— 出口改由三处提供：泳道「更多」菜单里新加的**「退出工作台」**
    > （走 `WorkspaceNavigationBridge.exitWorkspace()` 回到页面那两条判断）、
    > `Esc`、触摸屏的系统返回键 / 侧滑。同一节里另有一颗**顶栏开关**，
    > 于是「把顶栏叫回来」不必进设置页。
    > **认下的代价**：沉浸四边栏模式没有泳道栏头，那边只剩 `Esc` 与系统返回键。
    > 判据在 `top_chrome_test.dart` 第 4 节（含「菜单里那颗按一下，这一页真的被弹掉」
    > 与「那颗开关端到端能把顶栏叫回来」），变异 M56–M58 钉这三处；
    > 实机清单在 `docs/workspace-top-chrome-acceptance.md`。
    > 老快照没这个键 ⇒ 字段级兜底退回默认，**升级后顶栏会消失**，这是刻意的。
