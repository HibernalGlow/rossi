...
滚轮方向词（`up` / `down`）默认按**内容往哪走**算，而 macOS 的系统「自然滚动」已经把
这层反转做进了 `scrollDelta.dy`：开着它的用户会觉得「向上滚怎么翻到了下一页」。
`invertWheelDirection` 抵掉这一次反转，让方向词改按**手往哪推**算，`null` 表示跟随平台
（macOS 开、其它平台关）。判定只有 `OperationBindingStore.wheelDirectionFollowsHand` 一处，
运行时采集、录制器采集与「绑定表关掉」那条旧硬编码滚轮都读它 —— 三处分开写就会出现
「录进去是 up、触发算 down」和「开/关总开关方向不一样」。它属于采集设置而不是映射数据，
**不进** `bindingsJson`，所以导入导出的绑定包不会把别人的手方向带过来。
...
排序，切换时沿 Neo 的分支遍历继续寻找书籍，完成后替换当前阅读目标。在文件管理器里
点开单张图片时，它所在的目录会被当作一本书（mImageViewer 口径），点开的那一张只是
这一本的起始页，因此上下本沿该目录的书目继续走；只有从书架或历史里带着旧的单图片
路径进来时，才仍按「一张页」处理、不跨书切换。到达边界时给出提示。「切换书库」仍是
待接入动作。
