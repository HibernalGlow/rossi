...
2. **在这条路上，"降采样"反而是净亏。** 缩到 1024 要花 79.8 ms 重采样，只省下 21.3 ms
   的装箱 + 约 1 ms 的上传，**净多花约 58 ms**。`decode_rgba_scaled` 值得做，是因为
   **CPU 兜底路**要把 229.7 MB 过桥交给 `ui.decodeImageFromPixels`（那一段实测约 1260 ms，
   成本跟着位图 MB 走）；**GPU 路没有这次过桥**，像素直接写进 wgpu 纹理，所以那条理由
   在这条路上**不成立**。它在这里只有两个作用：绕开 **8192 的纹理单边上限**
   （本页 9504 > 8192，不缩根本建不出纹理）和压住显存占用。
   → **档位应当按"最大放得下的尺寸"选，而不是按显示宽度选**（尚未改，见 §7）。
...
- **预取（这条路现在最大的一个洞）**：本地核心那条路的预取已经落地（命中页 8–9 ms，
  见 `docs/v0.1-local-core.md` §12），但**呈现器完全不消费它** —— `should_prefetch_final_effect`
  未桥接，GPU 路上一个预取都没有。§6.3 量得很清楚：**除解码外全部工作只要 1.0 ms**，
  所以预取命中之后翻页就该是那个量级；而现在每一页都要现付 `dav1d 331 ms + 重采样 ~80 ms`，
  且**同步跑在 platform 线程上**（`show` 全程持锁，见 §2.2）—— 那 400 ms 里窗口连输入都不响应，
  观感比"慢"更糟。
  接的时候有三条**已经踩过**的约束必须一起带上（都出自本地核心那条路的实现）：
  **① 预取的 dav1d 要限线程**，否则会把翻页饿到 ~1 核（实测 550–648 ms，见 §6.2）；
  **② 按档位尺寸存**（一页 1276×850 的 RGBA ≈ 4.3 MB，3 页约 13 MB —— 不是 CPU 路那个
  179 MB 的坑，但要按目标宽度做键）；**③ 目标尺寸一变就整批作废**。
  本地核心那条路还有一条经验适用：**先等 ~180 ms 再起预取**，连翻时用 generation 让它自动退场，
  免得连点十几页时白解一堆。
- **档位按"显示宽度"选是错的（见 §6.3 结论 2）**：这条路上降采样净亏约 58 ms。
  应当改成在 `MAX_EDGE`（8192）与显存占用之间取"最大放得下的尺寸"，而不是当前的
  `target.width.min(MAX_EDGE)`。
- ~~**预取与呈现的联动**~~ → 已并入上面第一条。
