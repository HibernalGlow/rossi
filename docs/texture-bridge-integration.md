...
### 5.4 无人值守翻页量具（帧时间分布只能靠它）

`lib/gpu/page_turn_probe.dart` ＋ `.workbuddy/tools/run_page_turn_probe.py`。
设 `ROSSI_PAGE_TURN_LOG=<基路径>` 启动 App，它会**绕开整个正常启动**（不建数据库、
不连网、不还原窗口尺寸），直接进 GPU 上屏页 → 等呈现器就绪 → 打开来源 → 自动翻 N 页 →
写三个文件退出：汇总、`.turns.csv`（每轮的 present 往返 + 分段耗时 + **是否命中预取**）、
`.frames.csv`（每帧 build/raster/span）。

```bash
python .workbuddy/tools/run_page_turn_probe.py --sample "<某本>.zip" \
    --turns 10 --dwell 2500 --out .workbuddy/logs/xxx.txt
# 对照组：--no-prefetch（等价于 ROSSI_GPU_PREFETCH=0）
```

五个必须知道的坑：

1. **这个 exe 是 GUI 子系统，`print` 与未捕获异常不会出现在任何地方。** 量具的成功/失败
   **只认文件**（启动失败写 `<基路径>.boot-error.txt`）—— 实测连 `print(summary)` 都落不到
   重定向的文件里，所以别指望从 stdout 判断跑没跑。
2. **runner 有单实例守卫**（`windows/runner/main.cpp`）：只要已存在一个 `zephyr` 窗口，
   它就 `return EXIT_FAILURE` —— 表现是 **rc=1、无输出、无文件**，与"启动就崩"分不开。
   量具靠 `ROSSI_PAGE_TURN_LOG` 绕开守卫（见那儿的注释）。
3. **进程可能不退出，而且这与预取无关**（证据见 §6.4 边界）。所以启动器给退出设了上限
   （`--timeout`）：超时强杀，**产物优先** —— 三份文件齐全就算这一份能用。
   曾经因为"等不到退出就判失败"白丢一整轮数据。
4. **轮数多于页数会绕回**（`turn % pageCount`）。夹在最后一页会变成"把同一页反复交出去"，
   量到的是幂等早退、不是翻页；而判据 C 要 100 次，40 页的书必然绕回。
5. **判据 C 只在 Release 下成立**，而 `flutter test` 只能跑 Debug ⇒ 量具必须长在 App 自己
   身上。但拿它比"改动前后"时，同一 profile 内自洽即可，Debug 够用。

两个口径（都会影响读数，所以写明）：

- **判据 C 不含第 1 轮。** 第 1 轮是"打开来源后的首屏"，属冷启动（判据 B 的地界）；
  把它算进 p95 会把判据直接判死（实测含它 p95 = **34.71 ❌**，不含是十几毫秒）。
  排除，但汇总里**单独报一节**，不藏。
- **汇总另报"帧跨度"**（`vsyncStart → rasterFinish`，含排队等待）。它不是"这一帧花了
  多久"，而是"这一帧被推迟了多久" —— **"窗口冻没冻"只在这个数上暴露**，因为 native
  在 platform 线程上同步解码时，`frameMs` 看着仍然正常（对照组最大 487.7 ms 就在这儿）。
...
### 6.3 真实内容一页的账：60.2 MPix AVIF，396 ms 里 395 ms 是解码

这一轮是**用户报"翻页慢了"**追出来的，样本是真实漫画（不是低熵测试图）。
归档：`D:/1Dev/tmp/017. …[40P-545MB] [艾西 aiwest].zip` —— 40 页全 AVIF、
`9504×6336` = **60.2 MPix**、每页约 3.2 MB，而且**横竖混排**（第 21 页起是 `6336×9504`）。

界面读数（GPU 上屏页，第 5 / 40 页）：

| 分段 | 耗时 |
| --- | --- |
| **解码** | **395.4 ms** |
| 上传 | 0.3 ms |
| 渲染 + 提交（含 GPU→GPU 拷贝） | 0.7 ms |
| **合计** | **396.5 ms** |

`cargo run -p rossi_local_core --bin scale_probe -- <归档> 4` 把"解码"这一项拆开
（每段取 3 轮最小值；dev profile，但 dav1d 是 C，profile 不影响这一段）：

| 档位 | dav1d | 装箱 `to_rgba8` | 缩放 | 合计 | 位图 |
| --- | --- | --- | --- | --- | --- |
| 全尺寸（基线） | 331.1 | 21.6 | 0.0 | **352.7** | 229.7 MB |
| thumbnail → 4096 | 331.1 | 4.4 | 182.5 | 518.0 | 42.7 MB |
| thumbnail → 1440 | 331.1 | 0.5 | 86.5 | 418.2 | 5.3 MB |
| thumbnail → 1024 | 331.1 | 0.3 | 79.8 | **411.2** | 2.7 MB |
| thumbnail → 768 | 331.1 | 0.2 | 75.4 | 406.7 | 1.5 MB |

三条结论：

1. **dav1d 那 331.1 ms 是固定成本，一毫秒都不会因为档位变小而减少。** AVIF 没有可直接
   使用的 DCT 缩放档（JPEG 的 1/8–1/1 那种），要缩放就得先解出全尺寸 —— 这条结论
   `decode.rs` 的文档注释里早就写了，这里是它的第三个独立验证（前两次是 44.8 MPix 的
   5464×8192 与屏幕截图那批）。**别再重新发现一遍。**
2. **降采样省的不是解码，而是"解码之后"的东西 —— 两条路上它买的不是同一样东西。**
   缩到 1024 要花 79.8 ms 重采样，换来的是：装箱从 21.6 ms 降到 0.3 ms、位图从 229.7 MB
   降到 2.7 MB。
   - **CPU 兜底路**：那 227 MB 要过桥交给 `ui.decodeImageFromPixels`（实测约 1260 ms，
     成本跟着位图 MB 走）→ 这一步是决定性的，收益 8 倍以上。`decode_rgba_scaled` 的
     全部理由都在这里。
   - **GPU 路**：没有这次过桥，上传也只省约 1 ms → **只论时间，这 80 ms 是净亏约 58 ms**。
     但**不能因此把它删掉**，这是这一轮最容易得出的错误结论：
     着色器那次缩小是**双线性**的，9.6 倍缩小会明显走样；而 `thumbnail` 走 box 平均，
     是这条链路上**唯一在做正确缩略**的地方。它还顺带把页纹理从 179 MB 压到 5.3 MB，
     并绕开 8192 的纹理单边上限（本页 9504 > 8192，不缩根本建不出纹理）。
     → 正确的结论是：**保留它，但把它当成关键路径上的固定成本** —— 预取要连它一起藏。
     （真想去掉它，该做的是上传大图 + 生成 mip 链、让 GPU 侧做正确的缩小，
     那属于 Phase 2 tile 化；把 `hint` 改大只是把走样和显存一起放大。）
3. **除解码外的全部工作只要 1.0 ms**（上传 0.3 + 渲染提交 0.7）。这既说明 GPU 这条路
   本身没有问题，也说明**它的上限完全由"这一页什么时候被解开"决定**。

> **硬件解码再确认一次：仍然是死的。** 上一轮否掉硬解的依据是"那本 AVIF 是 4:4:4"，
> 这次担心是单本偶然，于是对**这批内容**抽了三页（第 1 / 5 / 21 页）实测：
> `ffmpeg -i` 一律报 **`yuv444p`**（AV1 **High** profile）。AV1 硬件解码器只吃 4:2:0，
> 所以"典型 AVIF 是 4:4:4"**不是一本的特例，而是这类内容的常态**。
> 顺带确认：`av1_cuvid` 那类路径不需要再试了。

> **界面上两行读数对不上，是显示问题，不是数据问题。** 截图里 `解码档位 1276 × 850`
> 与 `目标尺寸 850 × 670` 并存 —— 按 `hint = target.width` 算，850 宽只该解成 `850×566`。
> 原因：`decodedWidth/Height` **只在 `show` 时更新**，而 `目标尺寸` 每次 `resize` 都更新。
> 于是窗口缩小之后、还没翻页时，两行来自不同时刻。1276 = 850 × 1.5 正好是 DPR，
> 才显得像"DPR 被乘了两次"。**要修就让"档位"跟着"上次呈现时的目标"一起显示**
> （或与当前目标不一致时标注出来），别让两个不同时刻的数并排站着。


### 6.4 接上预取之后：Release 下翻页中位 479 ms → 16 ms

§6.3 的结论是「除解码外只 1.0 ms，所以唯一杠杆是预取」。这一节是接上预取之后的账。

**怎么量的**：`.workbuddy/tools/run_page_turn_probe.py`（无人值守翻页量具，见 §5.4）。
同一本 60.2 MPix AVIF（40 页、`yuv444p`）、每页停 2500 ms、**同一份 Release 产物**，
唯一差别是 `ROSSI_GPU_PREFETCH`（关 = 对照组）：

| 读数 | 关（对照） | 开 |
| --- | --- | --- |
| present 往返 **中位** | **479 ms** | **16 ms** |
| present 往返 范围 | 362–538 ms | 命中 7–29 ms；未命中 454 / 494 ms |
| 命中预取 | 0 / 40 | **38 / 40** |
| 每轮解码 | 341–515 ms | **0.0 ms**（命中时） |
| 判据 C p95 / p99 / max | 10.24 / 20.30 / 21.55 ✅ | 7.94 / 12.35 / 12.92 ✅ |
| 帧跨度 > 100 ms 的帧 | 3 | 1 |

**中位 479 → 16 ms（约 30 倍）**。没命中的两轮：第 1 轮（还没有"上一页读过"这件事
可供预取）与第 11 轮（唯一一次没赶上，494 ms）。

> Debug 下同一组（10 轮）是 469 → 6 ms、9/10 命中。两处"命中后"读数差 6 vs 16 ms，
> 差距全在 **渲染提交**（Debug 1.0–2.6 ms、Release 4–20 ms）—— 同一段代码在 Release
> 反而慢一个量级，**原因未查**（嫌疑是 GPU 队列与引擎合成争用，不是本路径的 CPU 工作）。
> 它不影响结论：16 ms 与 479 ms 的量级关系不受影响。

#### 它为什么能成：三条纪律

1. **不排队，只有一格锚点。** 共享状态只表达"用户现在停在第几页"；预取线程每 50 ms
   醒一次，按交互次序（`+1, -1, +2`）挑**第一个还没缓存**的页，解一页。
   排队是已经踩过的坑：连翻 5 页就积 5 个 400–500 ms 的解码任务，呈现线程随后要的解码
   和这些积压抢核，把翻页拖到 550–648 ms —— §6.2 那次的真因就是"两个 auto=16 并发"。
2. **准入判决与目标次序都取自本地核心**，这边不另立一套：
   `decide_prefetch_allowed(now, 上次翻页时刻, 当前页是否还没出图)` ＋
   `interleaved_prefetch_positions(anchor, n, 2, 1)`。
   两条都是从 mImageViewer 搬来的纯函数，而 Rossi 这边「滚动」= 翻页、
   「可见区待完成」= 当前页还没出图 —— 语义完全对上（`prefetch_policy` 模块文档里写着）。
   自己写「延迟 180 ms + 一个布尔」只会得到它的退化版（本地核心那条路的口径是
   **100 ms 静默 + 3 s 兜底**，还带拦截理由）。
3. **缓存条目按 `(页下标, 解码档位, 来源代次)` 做键。** 换来源代次变、拖窗口档位变 →
   都不命中。档位必须一起做键：旧档位的结果既不合观感（页比目标大一截），
   省下的时间也白省。解码全程**不持锁** —— 持着锁解一页，呈现线程来取缓存会被挡掉
   整整一页的时间。

#### 连翻极限：每页停 500 ms、100 轮 —— 预取追不上，而且这是设计

| 跑序 | 组 | 命中 | present 中位 | present p95 | present 最大 |
| --- | --- | --- | --- | --- | --- |
| 1 | 先跑：**开** | 2 / 100 | 467 ms | 504 ms | 514 ms |
| 2 | 后跑：**关** | 0 / 100 | **640 ms** | 1138 ms | 1375 ms |
| 1 | 先跑：**关** | 0 / 100 | **445 ms** | 483 ms | 499 ms |
| 2 | 后跑：**开** | 1 / 100 | 450 ms | 496 ms | 551 ms |

1. **预取在这条节奏下基本不出手 —— 这是对的。** 100 轮里后台只解了 3 页（命中 1–2 次），
   因为准入判决里「当前页还没出图」在 500 ms 停留 ＋ 467 ms 解码下**几乎恒为真**，
   它一直在让路。用户在等这一页时抢核去解下一页，只会两头都慢。
2. **开关在连翻下量不出可辨差异。** 同一个"关"配置：第一个位置 **445 ms**、第二个位置
   **640 ms**（差 +43%）—— 所以第一对里那个"关更慢"是**跑序效应**，不是预取的效果。
   本机连续 AVIF 解码几分钟后，第二个位置会系统性偏慢。
   → 纪律：**同一份产物的 A/B 必须正反各跑一次**，否则会把降频读成特性。
3. 500 ms 这条节奏代表"快速连翻"，**不是阅读节奏** —— 阅读节奏看前面那张表。

#### 判据 C 在 Release 下过了 —— 但它现在量不到这件事

Release 40 轮那组：p95 **7.94** / p99 **12.35** / max **12.92** ms，三条全过。
**对照组（没有预取）也全过**：p95 10.24 / p99 20.30 / max 21.55 ms。

这不是"预取没用"，是**口径的盲区**：翻页发生在 Dart 帧管线**之外**（native 解码 →
纹理交换 → 引擎合成），所以 `buildDuration + rasterDuration` 在两组里都只有 5–10 ms，
用户感觉到的那 479 ms 完全不在这个数里。跟着走的其实是另外两个读数：

- **present 往返**：479 → 16 ms（这是"用户等了多久"）；
- **帧跨度**（`vsyncStart → rasterFinish`，含排队等待）：对照组最大 **487.7 ms**，
  即窗口真的被 platform 线程冻住过；预取组那条只剩第 1 轮。

→ 建议（**没有改** `docs/v0.1_acceptance.md`，那是冻结的判据）：判据 C 应补一条
「翻页完成时间」的 p95，否则"链路很顺、每页等半秒"的状态能过判据。

#### 边界（这几条还没解决）

- **每页停留短于约 0.65 s 时预取追不上 —— 而且它选择不追**（准入静默 100 ms ＋
  一页解码 450–550 ms）。实测：`dwell=1500 ms`（Debug）第 1、2 轮没命中；`dwell=2500 ms`
  只剩第 1 轮；`dwell=500 ms` 的 100 轮只命中 2 次、后台一共只解了 3 页 —— 因为
  「当前页还没出图」在这条节奏下**几乎恒为真**，准入判决一直拦着它。**这是设计不是故障**
  （用户在等这一页时不抢核）；代价是连翻时退回"每页现付 467 ms"。
  Debug 下曾量到追不上时轻微变慢（+7%），Release 的 A/B 因为两次跑先后进行、排除不了降频，
  **没当结论**（见上面「连翻极限」第 2 条）。真要改，方向是换策略而不是限线程数。
- **`show` 仍然同步持锁**（§2.2）。命中预取时它只花 4–29 ms，所以常见的翻页已经不冻
  窗口；但**冷页照冻 400–500 ms**（刚打开、往回跳很远）。真解要动 §2.2 那条。
- **`dart:io exit()` 在 native 线程还活着时会异常退出。** 这一条与预取**无关**
  （2 轮微型样本照样复现），Release/Debug 一样，产物**已经写完**之后才发生：
  要么进程不退出（Release 40 轮那次只剩 1 个线程、0% CPU，挂了 7 分钟），
  要么退出码是 `0xE0464645`。原因是 `exit()` 绕过了 C++ 桥的析构，于是在
  CRT/DLL 卸载阶段与还活着的 native 线程（预取线程、wgpu/dav1d 的线程池）撞上。
  量具因此改成"**超时强杀 ＋ 产物优先**"：退出码不是交付物，产物才是（见 §5.4）。
  产品关窗走的是**另一条路**（C++ 桥析构 → `rossi_gpu_present_destroy` → `Drop` 里
  `stop` ＋ `join`），没有走这条；但"关窗时预取正好在解码"还没实测过 —— 记为待办。
...
- ~~**判据 C 还没量**~~ → **已量（2026-09-17，Release，40 轮真实内容），但口径有盲区**：
  两组（预取开／关）的三条阈值**全过**（开 7.94 / 12.35 / 12.92 ms，关 10.24 / 20.30 / 21.55 ms）。
  **它过了不代表翻页不慢** —— 翻页发生在 Dart 帧管线之外，用户感觉到的那 479 ms 不进这个数；
  跟着走的是 present 往返与帧跨度（见 §6.4「判据 C 在 Release 下过了」）。
  给判据 C 补一条「翻页完成时间 p95」是**决策**，所以没有动 `docs/v0.1_acceptance.md`。
...
- ~~**预取（这条路现在最大的一个洞）**~~ → **已做（2026-09-17），见 §6.4**：
  Rust 侧一条后台线程按本地核心的准入判决与目标次序解码一页、存进有界缓存
  （3 页 / 48 MB，键含档位与来源代次），`show` 命中就直接上传。
  实测 Release 40 页真实内容：present 往返中位 **479 → 16 ms**、38/40 命中、命中时解码 0.0 ms。
  剩下的四条见 §6.4 的「边界」：
  ① 停留短于 ~0.65 s 时**追不上、而且故意不追**（让路给正被等的那一页）；
  ② `show` 仍同步持锁，冷页照冻 400–500 ms（§2.2）；
  ③ 判据 C 的口径漏掉翻页本身（见上一条）；
  ④ `exit()` 绕过桥析构会异常退出（量具侧已用超时＋产物优先兜住）。
- **连翻节奏下的策略**：现在 `dwell ≤ 0.65 s` 等于不预取。一个待验的想法是——连翻时改成
  "只保留最近解过的那几页"（复用已有结果）而不是"预取下一张"。注意这**不是**"限解码线程数"
  能解决的：拦它的是准入判决，不是抢核。
- **"关窗时预取正好在解码"未实测。** 产品路径是 `Drop` 里的 stop ＋ join，按构造是安全的
  （最多等一页解码），但没人真的在那一点关过窗。需要手动做一次：打开 GPU 上屏页 → 立刻关窗。
- ~~**档位按"显示宽度"选是错的**~~ → **这条判断我当天就改过一次，把过程留着**：
  一开始只算了时间（重采样 80 ms 换回 22 ms），于是得出"该按最大放得下选"。
  那是错的 —— 漏了缩小**质量**（着色器是双线性、`thumbnail` 是 box 平均）和**显存**
  （179 MB vs 5.3 MB）。当前的 `target.width.min(MAX_EDGE)` 是合理的。
  更正与完整理由见 §6.3 结论 2；真要去掉这次缩小，得走 mip 链那条路。
- ~~**预取与呈现的联动**~~ → 已并入上面第一条。
