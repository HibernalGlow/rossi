...
class DownloadTaskRepository {
  ...
  /// 本漫画的全部任务（单章节任务模型下一本漫画可能同时有多个任务）。
  List<DownloadTask> findTasksForComic({
    required String from,
    required String comicId,
    bool incompleteOnly = false,
  }) {
    final normalizedFrom = from.trim();
    final normalizedId = comicId.trim();
    return getAll(incompleteOnly: incompleteOnly).where((task) {
      final payload = readPayload(task);
      if (payload == null) return false;
      return payload.from.trim() == normalizedFrom &&
          payload.comicId.trim() == normalizedId;
    }).toList();
  }
