...
mod mac_platform {
    ...
    /// 设置 HDR 逆色调映射模式与参数。
    /// mode: 0 (关闭), 1 (扩展线性 HDR), 2 (SDR 增强)
    /// boost: 白点/高光倍率 (1.0 ~ 16.0, 默认 1.5 ~ 3.0)
    /// peak: 显示器峰值倍率 (1.0 ~ 16.0, SDR 为 1.0, EDR 为 2.0 ~ 10.0)
    #[no_mangle]
    pub extern "C" fn rossi_gpu_present_set_hdr(
        presenter: *mut c_void,
        mode: u32,
        boost: f32,
        peak: f32,
        ...
        let cfg = match mode {
            crate::wgpu_resampler::HDR_EXTENDED_LINEAR => {
                crate::wgpu_resampler::HdrConfig::extended_linear(boost, peak)
            }
            crate::wgpu_resampler::HDR_SDR_BOOST => {
                crate::wgpu_resampler::HdrConfig::sdr_boost(boost)
            }
            _ => crate::wgpu_resampler::HdrConfig::OFF,
        };

        match inner.set_hdr(cfg) {
            Ok(()) => 0,
            Err(e) => {
                write_err(err_buf, err_len, &format!("{e:#}"));
                -1
    ...
    /// 切换输出通路。`mode`: 0 = Flutter 外部纹理（8 位，只能 SDR）；
    /// 1 = EDR 平台视图（半精度线性浮点，可以真 HDR）。
    ///
    /// 两条通路的像素格式不同，所以切之前必须让上屏端先准备好对应格式的缓冲区；
    /// 否则就是拿 8 位的假设去解 16 位的数据。
    #[no_mangle]
    pub extern "C" fn rossi_gpu_present_set_output_mode(
        presenter: *mut c_void,
        mode: u32,
        err_buf: *mut u8,
        err_len: usize,
        ...
        match inner.set_output_mode(mode == 1) {
            Ok(()) => 0,
            Err(e) => {
                write_err(err_buf, err_len, &format!("{e:#}"));
                -1
    ...
    /// 获取呈现器当前的输出格式单像素字节数 (4: BGRA8, 8: RGBA16F)
    #[no_mangle]
    pub extern "C" fn rossi_gpu_present_output_bpp(presenter: *mut c_void) -> i32 {
        let Some(holder) = (unsafe { borrow(presenter) }) else {
            return 4;
        };
        let guard = lock_slot(&holder.slot);
        if let Slot::Ready(inner) = &*guard {
            inner.output_bytes_per_pixel() as i32
        } else {
            4
        }
    }
