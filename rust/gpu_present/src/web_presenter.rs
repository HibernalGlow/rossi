...
impl RossiWebPresenter {
    ...
    pub async fn init(canvas_id: String, width: u32, height: u32) -> Result<RossiWebPresenter, JsValue> {
        let window = web_sys::window().ok_or_else(|| JsValue::from_str("无法获取 window 对象"))?;
        let document = window.document().ok_or_else(|| JsValue::from_str("无法获取 document 对象"))?;
        let element = document.get_element_by_id(&canvas_id)
            .ok_or_else(|| JsValue::from_str(&format!("未找到 id 为 {canvas_id} 的 canvas 元素")))?;
        ...
        let resampler = WgpuResampler::new_with_format(Arc::clone(&device), Arc::clone(&queue), format)
