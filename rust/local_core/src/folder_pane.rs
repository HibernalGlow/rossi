...
//!
//! **刻意偏离**：全部为了让它在另一个 crate 与跨平台环境中可用，登记在
//! `script/sync_vendored_modules.py` 的 `PORTS` 表中。
//!
//! 1. `pub(crate)` → `pub`（机械替换）。
//! 2. `crate::known_folders::available_drives()` → 委托为本模块 `available_drives()`（调用 `crate::file_tree::get_available_roots()` 跨平台根路径）。
//! 3. `crate::perf::is_enabled()` / `crate::perf::event(...)` → `crate::perf_sink::` 的同名函数，`serde_json::Value` → [`PerfValue`]。
//! 4. 增加了跨平台 Unix 路径回归测试（上游原生测试标记 `#[cfg(windows)]`，Unix 上使用对应的 `/` 路径验证等价行为）。
//!
//! 除这些，**函数名、类型名、常量名、测试名全部与上游一致**。
...
pub enum FolderPaneCommand {
    Open(PathBuf),
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FolderPaneRow {
    pub path: PathBuf,
    pub depth: usize,
    pub expanded: bool,
    pub loading: bool,
...
pub struct FolderPaneScanPending {
    key: String,
...
impl FolderPaneState {
    ...
    pub fn set_focus_tree_at_active(&mut self) {
    ...
    pub fn set_focus_grid(&mut self) {
    ...
    /// (= 単に閉じる)。
    pub fn cursor_nav_target_if_moved(&self) -> Option<PathBuf> {
    ...
    pub fn visible_rows(&self) -> Vec<FolderPaneRow> {
    ...
    pub fn set_cursor(&mut self, path: PathBuf) {
...
pub fn scan_real_subfolders(
    ...
    let perf_start = crate::perf_sink::is_enabled().then(std::time::Instant::now);
...
fn emit_folder_pane_scan_perf(
    ...
    let mut fields = vec![
        ...
        ("dirs_returned", PerfValue::from(stats.dirs_returned)),
        ("entry_errors", PerfValue::from(stats.entry_errors)),
        ("file_type_errors", PerfValue::from(stats.file_type_errors)),
        ("mtime_errors", PerfValue::from(stats.mtime_errors)),
        ("canceled", PerfValue::from(stats.canceled)),
        ("use_mtime", PerfValue::from(use_mtime)),
        ("sort", PerfValue::from(format!("{sort_order:?}"))),
        ("ok", PerfValue::from(result.is_ok())),
    ];
    if let Err(err) = result {
        fields.push(("error_kind", PerfValue::from(format!("{:?}", err.kind()))));
    }
    let key = path.to_string_lossy();
    crate::perf_sink::event(
        "folder_pane",
        "scan_subfolders",
        Some(key.as_ref()),
        0,
...
pub fn active_filesystem_folder(path: &Path) -> Option<PathBuf> {
...
mod tests {
    ...
    #[cfg(not(windows))]
    #[test]
    fn unix_active_virtual_folder_maps_to_parent() {
        assert_eq!(
            active_filesystem_folder(Path::new("/books/vol.zip")),
            Some(p("/books"))
        );
        assert_eq!(
            active_filesystem_folder(Path::new("/docs/scan.pdf")),
            Some(p("/docs"))
        );
    }
    ...
    fn unix_sync_to_active_expands_minimum_ancestor_chain() {
        ...
        let c_key = key_for(Path::new("/a/b/c"));
        assert!(state.auto_expanded.contains(&root_key));
        assert!(state.auto_expanded.contains(&a_key));
        assert!(state.auto_expanded.contains(&b_key));
        assert!(!state.auto_expanded.contains(&c_key));
        assert_eq!(state.active_path(), Some(Path::new("/a/b/c")));
    }
    ...
    #[cfg(not(windows))]
    #[test]
    fn unix_keyboard_moves_visible_rows_and_enter_opens_cursor() {
        let mut state = FolderPaneState::default();
        let root = p("/");
        let a = p("/a");
        let b = p("/b");
