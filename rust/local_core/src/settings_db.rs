...
impl SettingsDb {
    ...
    pub fn load_settings(&self) -> Result<Settings> {
        ...
        match maybe_json {
            Some(json) => {
                let settings = serde_json::from_str::<Settings>(&json)
...
pub fn resolve_view_state_for_path(
    path: &Path,
    states: &HashMap<String, FavoriteViewState>,
) -> Option<(String, FavoriteViewState)> {
    let mut best_key: Option<String> = None;
    let mut best_len = 0usize;

    for key in states.keys() {
        let saved_path = Path::new(key);
        if path.starts_with(saved_path) {
